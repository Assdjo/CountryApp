...
async function fetchCountry() {
  ...
  for (let i = 0; i < country.value.length; i++) {
   total.value = total.value + country.value[i].population
}
...
<template>
  ...
  <main>
    ...
    <article class="description">
      ...
      <p><b>Langue :</b> {{ Object.values(country[id]?.languages)[0] }}</p>

      <p>
        <b>Monnaie :</b><br />
        {{ Object.values(country[id]?.currencies)[0].name }}({{
          Object.values(country[id]?.currencies)[0].symbol
        }})
      </p>
    ...
    <article class="Infogenerale">
      ...
      <p><b>population totale :</b> {{ total }}</p>
    </article>
  </main>
  <footer></footer>
</template>
